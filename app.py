...
# 1. Page UI 구성 (사이드바 기본 펼침 설정 추가)
st.set_page_config(
    page_title="연구소 HW팀 부품 재고 관리", 
    page_icon="📦", 
    layout="wide",
    initial_sidebar_state="expanded" # 사이드바가 항상 펼쳐져 있도록 설정
)
...
# ----------------------------------------------------
    ...
    if not st.session_state["is_admin"]:
        ...
        reg_tab, edit_tab = st.tabs(["➕ 신규 부품 등록", "📝 기존 부품 정보 수정"])
        ...
        with edit_tab:
            ...
            if df_comp.empty:
                ...
                selected_edit_pn = st.selectbox("수정할 부품 (P/N) 선택", p_list, key="tab_edit_sel")
